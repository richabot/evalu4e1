...
// const [Findit,setFindit]=useState()
  ...
  return (
    <li data-cy="task" className={styles.task}>
     <input className={styles.check1} type="checkbox" data-cy="task-checkbox"  checked={isCompleted}
            onChange={(e)=>{
checking()
    ...
    <span style={{textDecoration:isCompleted ? 'line-through':'none'}}>  {props.item.text}</span>
      ...
      <button data-cy="task-remove-button"  className={styles.remove1} onClick={()=>  props.onDelete(props.item.id)}> <img src={image} alt="" /></button> 
